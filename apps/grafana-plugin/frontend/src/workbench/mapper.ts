...
export function ChartWireToDataFrame(series: ChartSeriesWire[]): DataFrame {
  const timestamps = series[0]?.points.map((point) => new Date(point.timestamp).valueOf()) ?? [];
  return createDataFrame({
    name: 'node_exporter mock',
    fields: [
      { name: 'Time', type: FieldType.time, values: timestamps },
      ...series.map((item) => ({ name: item.name, type: FieldType.number, labels: item.labels, values: item.points.map((point) => point.value) })),
    ],
  });
}
