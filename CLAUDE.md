...
- 实现工作必须采用小步提交：每个独立且可验证的职责切片完成后立即创建一个 commit；提交前运行与该切片相称的最小验证，并在提交信息中准确说明内容。不要把多个 Gate、无关格式化或用户已有修改混入同一提交，也不要为“攒大提交”延迟已验证的变更。
- 用户已授权本项目的实现过程自动创建上述小步 commit；除非用户另行要求，不自动 push、创建 PR、amend 或改写历史。
