...
|`services/ai-core/internal/domain`|核心领域模型和状态规则：private/org_incident Session、user/assistant/trigger Message、metric_analysis/incident_remediation Task、摘要 AlertEvent、TaskEvent/ToolCall、Chart/Execution、处置 Intent/Approval/Execution/Audit、绝对时间范围、有效 QueryPlan 与领域错误。|两类 Task 使用隔离状态机；IncidentPlan 只接受固定资产、结构化诊断和精确 `0 -> 2` Intent。Approval 只有 pending→approved/rejected/expired 单次转换且固定十分钟期限；Execution 只接受精确版本回执或进入 unknown 后只读 reconcile；Audit action/outcome 是闭集。|
|`services/ai-core/internal/application` 与 `internal/ports`|命令服务、指标分析、告警 Incident、可恢复诊断/prepare 和 Approval 决策服务；Port 定义存储、通知、Agent、工具、时钟、ID 及处置生命周期仓储等能力。|prepare 原子冻结 Intent 并创建十分钟 Approval。Approval service 复验 org/Task/Intent/TaskVersion/ApprovalVersion，只有 Admin 可用幂等键作单次决定；approve 仅调度后续工作流，reject/expired 原子取消，三者先持久化事件和权威 Audit。并发 CAS 仅一路成功，幂等重放不重复调度。|
|`services/ai-core/internal/adapters`|SQLite、内存通知器、MCP 客户端、系统时钟/随机 ID、Mock/Eino Agent、Grafana ingress 和生成式 HTTP API Adapter。Incident Toolset 暴露 resolve/start、四调用只读观察和确定性 Playbook prepare。|生成 HTTP Approval handler 已实现严格 JSON、完整身份/header 转换及 200 GET/202 decision；应用服务未注入时继续 fail-closed。bootstrap 将在受控执行工作流就绪时才同时接通 Approval，避免可批准但无执行器的运行态。|
|`services/ai-core/internal/bootstrap`|组装依赖：SQLite Store、Mock 或显式 opt-in Eino/DeepSeek Agent、MCP Gateway、两类工作流、可选 Incident 服务和 HTTP API。|未提供 Webhook secret 时 Incident 入口关闭；启用时固定 source/tenant/org/system actor、恢复未完成诊断，并在 `/readyz` 额外检查 Knowledge/Skill/Playbook 与完整 order-service remediation profile；密钥只来自环境且不持久化。|
|`services/assistant-mcp`|以 Streamable HTTP（`/mcp`）暴露闭集 MCP 工具。默认仍只有三个只读 `grafana.*`；incident diagnosis profile 增加 11 个只读工具，显式 remediation profile 再增加唯一 typed `0 -> 2` 写工具、固定恢复指标 view 和业务 probe。|诊断与处置分别要求 `incidents:diagnose`、`incidents:remediate`；写路径还要求短期 ApprovalEvidence、独立目标写 token 和 append-only JSONL audit。工具与 Port 均无 Fault、shell、任意 HTTP 或通用 execute。该服务不拥有 AI Core Task/Approval/数据库；其本地执行 audit 是跨边界防御记录，不取代 AI Core 权威审计。|
...
|`services/assistant-mcp/internal/ports/incidentmetrics` 与 `adapters/incidentmetrics`|处置验证专用的单一注册 Prometheus recovery view。|调用者不能提交 PromQL、指标名、范围、endpoint 或查询参数；HTTP Adapter 只执行固定 accepted/completed 30 秒增量、queue depth 和 oldest age 四个 instant query，逐个限制 64 KiB、唯一有限数值结果。Mock 返回确定性恢复事实；已由 remediation profile 暴露，等待 AI Core 验证工作流调用。|
|`services/assistant-mcp/internal/ports/orderdemo` 与 `adapters/orderdemo`|订单 Operational API 的类型化抽象及 Mock/HTTP 实现；诊断 `Port` 与执行专用 `RemediationPort` 分离。|诊断 Port 只包含 runtime、queue、worker、policy、有限 recent outcomes 和 operation reconcile；RemediationPort 只有固定 `0 -> 2` CAS 与业务 probe。HTTP Adapter 使用不同读写 token、固定超时、禁止重定向、64 KiB 响应上限及领域语义复验；Mock 保留四类诊断证据并实现同语义 operation 幂等。没有 Fault、任意 HTTP 或命令方法。|
...
|`compose.incident-e2e.yaml`、`deploy/grafana/provisioning`|可控订单故障的真实监控、诊断与受控修复拓扑。|order-service 分别接入 business/ops/metrics internal network；assistant-mcp 仅加入 ops，以不同 token 访问只读/修复端点，并将执行边界 audit 写入独占 volume；fault-controller 使用共享 Unix Socket 且 `network_mode: none`。Prometheus 5 秒抓取，Grafana 以不可编辑 datasource、10 秒评估的 `OrderQueueBacklog` 和带 HMAC 的 Webhook contact point 发现异常，AI Core 验签后创建可恢复组织 Incident。|

## 关键数据与依赖边界

- AI Core 独占业务持久化。SQLite 迁移在 `services/ai-core/migrations/sqlite/`，Plugin 和 MCP 都不能直接读写它。`0007` 建立 Task/Incident union、checkpoint 与摘要 AlertEvent；`0008` 增加不可变 RemediationIntent、十分钟单次 CAS Approval、先落盘且可 reconcile 的 RemediationExecution 和 append-only AuditRecord。触发器复验 tenant/org/Task/Intent/Approval/epoch/version 关联，未 approved 的 Approval 无法创建 Execution；迁移后执行 `foreign_key_check`，V1/V6 升级、重启与并发 CAS 均有回归测试。
...
|订单 Incident|Mock 基础 + real-metrics + `compose.incident-e2e.yaml`|既有指标 Agent + 确定性 Incident 诊断/prepare；独立 Remediation Toolset 已接入 durable write-once Execute/Reconcile/三重 Verify|Prometheus 抓取 node_exporter 与真实 order-demo；Grafana Alert firing/resolved|Docker；AI Core 已原子接通 Alert ingress、组织 Incident、Approval API、Execution/Audit 与诊断后执行恢复；Plugin 代理/UI 与真实批准 E2E 尚未完成|
