...
|`services/ai-core/internal/adapters`|将 Port 接到具体实现：SQLite、内存通知器、MCP 客户端、系统时钟/随机 ID、确定性 Mock Agent 与受限 Eino Agent。Incident Toolset 只暴露 resolve/start 和四调用只读观察。|诊断固定调用 queue/worker/policy/recent，摘要不保留订单 ID，并本地区分 stopped/slow/dependency/healthy/证据不足；无 Fault、operation、任意 HTTP 或写工具路径。Incident HTTP 新路由仍是 fail-closed 占位。|
