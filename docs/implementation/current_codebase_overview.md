...
|`services/ai-core/internal/application` 与 `internal/ports`|命令服务、有限查询意图解析、指标分析工作流、告警 Incident 服务和可恢复只读诊断工作流；Port 定义存储、事件通知、Agent、工具、时钟和 ID 等外部能力。|Alert firing 幂等创建组织 Session/trigger/Task/checkpoint 后异步诊断；只接受四个固定只读工具证据，状态、ToolCall、Diagnosis、checkpoint 和 TaskEvent 均先持久化再通知。两类重启恢复按 Task kind 分流，已完成诊断不会重放。|
