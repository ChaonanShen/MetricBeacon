...
|`services/ai-core/internal/domain`|核心领域模型和状态规则：private/org_incident Session、user/assistant/trigger Message、metric_analysis/incident_remediation Task、摘要 AlertEvent、TaskEvent/ToolCall、Chart/Execution、绝对时间范围、有效 QueryPlan 与领域错误。|两类 Task 使用隔离状态机；AlertEvent 仅接受有界 labels/身份/时间/Task 关联，不承载原始 webhook、values、URL 或 ground truth；IncidentPlan 只接受固定资产、结构化诊断和精确 `0 -> 2` Intent。|
