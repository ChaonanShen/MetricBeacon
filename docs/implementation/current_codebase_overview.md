...
|`services/ai-core/internal/application` 与 `internal/ports`|命令服务、有限查询意图解析、指标分析工作流、告警 Incident 服务和可恢复只读诊断工作流；Port 定义存储、事件通知、Agent、工具、时钟、ID 及处置生命周期仓储等外部能力。|Alert firing 幂等创建组织 Session/trigger/Task/checkpoint 后异步诊断；只接受四个固定只读工具证据。新增 Intent、Approval、Execution、Audit 独立仓储 Port，供后续 prepare/approve/execute 工作流在同一应用事务内组合。|
