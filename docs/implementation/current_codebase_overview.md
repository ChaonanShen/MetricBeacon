...
|`services/order-demo`|可控订单业务系统的新 Go 模块。|订单状态机、100 容量队列、动态 worker、版本化配置、幂等提交、三种真实处理故障、受限 `0 -> 2` CAS、operation reconcile 和固定业务 probe 已在 Domain/Application 层实现并通过 race tests；HTTP、Prometheus 和容器 Adapter 尚未接线。Fault 合同不生成产品客户端。|
