...
|`services/ai-core/internal/domain`|核心领域模型和状态规则：private/org_incident Session、user/assistant/trigger Message、metric_analysis/incident_remediation Task、TaskEvent/ToolCall、Chart/Execution、绝对时间范围、有效 QueryPlan 与领域错误。|两类 Task 使用隔离状态机；IncidentPlan 只接受固定资产、结构化诊断和精确 `0 -> 2` Intent，相似故障只能 no-action。纯领域不依赖数据库、MCP、Grafana 或模型 SDK。|
