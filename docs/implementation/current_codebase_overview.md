...
|`services/ai-core/internal/adapters`|将 Port 接到具体实现：SQLite、内存通知器、MCP 客户端、系统时钟/随机 ID、确定性 Mock Agent 与受限 Eino Agent。Incident Toolset 只暴露 resolve/start 和四调用只读观察；Grafana ingress 是独立机器入口。|入口在 JSON 解析前校验 source、`timestamp:rawBody` HMAC、常量时间签名和 ±5 分钟窗口，限制 64 KiB/10 alerts 并严格拒绝未知字段；diagnosis 固定 queue/worker/policy/recent，无 Fault、operation、任意 HTTP 或写工具路径。Approval/list 路由仍 fail-closed。|
