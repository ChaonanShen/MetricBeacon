...
|`services/ai-core/internal/application` 与 `internal/ports`|命令服务、指标分析、告警 Incident、可恢复诊断与确定性 prepare 工作流；Port 定义存储、通知、Agent、工具、时钟、ID 及处置生命周期仓储等能力。|Alert firing 幂等创建组织 Incident；只接受四个固定只读诊断证据。诊断落盘后调用 pinned Playbook prepare：no-action 原子完成；唯一合法 Intent 则计算 canonical digest，并在同一事务冻结 Intent、创建十分钟 Approval、推进 Task/checkpoint 和追加事件。恢复器从已落盘 Diagnosis 继续 prepare，不重复诊断；Approval 前不创建 Execution。|
|`services/ai-core/internal/adapters`|将 Port 接到具体实现：SQLite、内存通知器、MCP 客户端、系统时钟/随机 ID、确定性 Mock Agent 与受限 Eino Agent。Incident Toolset 暴露 resolve/start、四调用只读观察和确定性 Playbook prepare；Grafana ingress 是独立机器入口。|diagnosis 固定 queue/worker/policy/recent；prepare 只把已持久化 Diagnosis 和 opaque checkpoint 交给 `playbook.resume_run`，接收版本/epoch/资产摘要绑定的 IntentDraft 或 no-action。该 Toolset 无 Fault、operation、ApprovalEvidence、任意 HTTP 或写工具路径。|
