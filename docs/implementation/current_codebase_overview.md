...
|`services/ai-core/internal/bootstrap`|组装依赖：SQLite Store、Mock 或显式 opt-in Eino/DeepSeek Agent、MCP Gateway、两类工作流、可选 Incident 服务和 HTTP API。|未提供 Webhook secret 时 Incident 入口关闭；启用时固定 source/tenant/org/system actor、恢复未完成诊断，并在 `/readyz` 额外检查 Knowledge/Skill/Playbook/order-service 只读 profile；密钥只来自环境且不持久化。|
...
|`services/assistant-mcp/internal/ports/incidentmetrics` 与 `adapters/incidentmetrics`|处置验证专用的单一注册 Prometheus recovery view。|调用者不能提交 PromQL、指标名、范围、endpoint 或查询参数；HTTP Adapter 只执行固定 accepted/completed 30 秒增量、queue depth 和 oldest age 四个 instant query，逐个限制 64 KiB、唯一有限数值结果。Mock 返回确定性恢复事实；尚待 G6 workflow 接线。|
