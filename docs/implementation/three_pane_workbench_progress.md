...
|P2：Mock 浏览器验收并收口|进行中|已更新 Playwright 用例，覆盖 1440px 三栏顺序、画布多列多行、详情联动、900px 纵向堆叠、刷新恢复和无水平溢出。2026-07-14 运行 `make e2e-mock` 时，前端构建及 typecheck 通过，但 Docker 绑定 `127.0.0.1:8081` 失败（port is already allocated）；未停止用户管理的占用栈，待端口释放后重试。|
