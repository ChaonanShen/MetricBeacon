...
|G6：审批、执行、验证与审计|进行中|Incident MCP 合同已增加唯一 typed `0 -> 2` 写工具、固定 30 秒 Prometheus 恢复视图和固定业务 probe；Go/TypeScript 生成物可复现。新增共享 ApprovalEvidence codec，以 60 秒上限、HMAC 和精确 tenant/org/task/approval/intent/capability/service/epoch/version/operation scope 防重放与串用，20 次重复、5 次 race 和 vet 通过。order-demo 现在按公开合同统一使用 `sha256:` Intent digest；assistant-mcp 将诊断 Port 与 RemediationPort 分离，HTTP Adapter 使用独立读写 credential，Mock/HTTP 均覆盖 CAS、operation 幂等、probe 和凭证隔离。Approval 持久化、证据接线、MCP write handler、指标验证和 AI Core 执行工作流仍待完成。|
