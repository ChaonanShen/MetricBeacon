...
|G6：审批、执行、验证与审计|进行中|Incident MCP 合同已增加唯一 typed `0 -> 2` 写工具、固定 30 秒 Prometheus 恢复视图和固定业务 probe；Go/TypeScript 生成物可复现。新增共享 ApprovalEvidence codec，以 60 秒上限、HMAC 和精确 tenant/org/task/approval/intent/capability/service/epoch/version/operation scope 防重放与串用。order-demo 现在按公开合同统一使用 `sha256:` Intent digest；assistant-mcp 将诊断 Port 与 RemediationPort 分离，HTTP Adapter 使用独立读写 credential。显式 remediation profile 才注册唯一写工具和两个验证工具，并要求独立 `incidents:remediate` 权限、ApprovalEvidence key、目标写 token 与本地 append-only JSONL audit；写前 audit 失败关闭，写后 audit 失败要求 reconcile，证据不进入 audit。固定 metrics view 与业务 probe 已接入。assistant-mcp 全测试重复 5 次、全模块 race 重复 2 次、vet、Compose config、36 Schema 和生成差异门禁通过。AI Core 的 Approval 持久化与执行工作流仍待完成。|
|G7：Plugin 与 Workbench|未开始|—|
|G8：E2E 与收口|未开始|—|

## 当前边界

G5 的告警接入、组织 Incident、持久化与可恢复只读诊断已完成；G6 的 assistant-mcp 执行边界已经就绪。下一步把确定性 prepare 产出的 Intent 接入 AI Core Approval、类型化 Execute、只读 reconcile、三重 Verify 和 AI Core 权威 Audit。
