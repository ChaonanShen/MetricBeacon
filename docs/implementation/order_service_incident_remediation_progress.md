...
|G5：AI Core 只读 Incident|进行中|领域层已加入显式 private/org_incident Session、trigger Message、metric_analysis/incident_remediation Task discriminator、相互隔离的状态机、固定资产 IncidentPlan、结构化 Diagnosis 与严格 `0 -> 2` Intent；slow/dependency/healthy/证据不足只能 no-action，指标 Task 不能进入审批态。SQLite `0007` 已无伪 QueryPlan 地重建约束表，保留旧 Message/Task/Event/Tool/Chart 图，新增组织/类型、opaque checkpoint 和摘要化 AlertEvent；活跃 fingerprint 唯一、checkpoint CAS/租户/仅 Incident、V1/V6 升级、重开恢复和 foreign key check 均有测试。AlertEvent 聚合/仓库只保留有界 labels、身份、状态、时间和 Task 关联，key 去重及跨租户拒绝已覆盖；原始 webhook/values/URL 不进入领域或数据库。HMAC 入口与只读工作流仍在后续小步。|
|G6：审批、执行、验证与审计|未开始|—|
