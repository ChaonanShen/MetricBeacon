...
|G6：审批、执行、验证与审计|进行中|Incident MCP 合同、共享 ApprovalEvidence、assistant-mcp 隔离的诊断/修复 Port、独立凭证、显式 remediation profile、两层执行 audit、固定 metrics view 和业务 probe 已接入并通过重复、race、vet、Compose、Schema、生成和冷构建门禁。AI Core 已新增闭集处置领域及 SQLite `0008`：不可变 Intent、十分钟 Approval 单次决定、Execution started/unknown/reconcile/terminal 回执和 append-only Audit；数据库触发器复验 tenant/org/Task/Intent/Approval/epoch/version，未批准时零 Execution，两路并发 CAS 仅一路成功，unknown 重启后可凭只读回执收敛且跨租户读取失败。AI Core 全测试重复 3 次，重点存储/领域重复 10 次、race 3 次及 vet 通过。Approval 应用服务和执行/验证工作流仍待完成。|
