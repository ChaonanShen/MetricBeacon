...
|G6：审批、执行、验证与审计|代码闭环已完成，待真实 E2E|Incident MCP/ApprovalEvidence/assistant-mcp 修复边界、AI Core 闭集领域、SQLite `0008` 和 durable prepare 已就绪。Approval 决策必须 Admin、Idempotency-Key、Task/Approval 双版本和精确 Intent digest。AI Core 独立 Remediation Toolset 已接入 durable 工作流：先持久化稳定 operation ID、Execution started 与 accepted Audit，再签发不落盘的 60 秒 ApprovalEvidence 并执行唯一 `0 -> 2` 写；任何不确定响应或 started 重启只调用 `get_operation`，绝不重放写；精确回执后依次验证 runtime/worker、两次相邻固定 30 秒 recovery view 和稳定幂等 probe ID 的真实业务探针。runtime→validating 与 metrics→business checkpoint 均原子推进，恢复可从 waiting/executing/reconciling/validating 精确续跑。bootstrap 仅在 webhook、至少 32 字节 Evidence key 都有效时，按诊断恢复→已批准执行恢复顺序原子注入 Approval API；readiness 同时携带 diagnose/remediate 权限检查完整 profile，AI Core 镜像显式纳入 Evidence 模块。G8 冷启动发现 readiness 曾用非契约单数 `skill` 查询实际 `skills.get_skill`，已修正为契约命名空间并新增精确工具计数回归。黄金路径、写后超时、started/unknown 重启、business checkpoint、回执篡改、receipt 缺失、指标失败、权限和 Evidence 不泄漏使用真实 SQLite 测试；workflow/MCP 重复 10 次、race 3 次、AI Core 全测试/vet、bootstrap 测试、Docker 构建及 Compose config 通过。|
|G7：Plugin 与 Workbench|已完成|Plugin Backend 已增加组织 Incident list、Approval GET/POST 受控代理；可信 Grafana identity 为所有角色增加 `incidents:read`，仅真实 Admin 增加 `incidents:approve`，并在代理边界提前拒绝 Viewer 写请求。tenant/org/role/permission 伪造头均被覆盖，page token 与 Idempotency-Key 原样受控转发。AI Core 新增 `tenant + org + org_incident Session` 联表的 recent-activity Task 游标，token 绑定 tenant/org；相同可见性覆盖 Incident Session/Message/Task/Event，而 private 仍严格 owner-only。Workbench 新增组织事件分页选择、结构化诊断及备选原因、证据/资产、不可变 `0 -> 2` Intent/Diff、Approval 状态、仅 Admin 可用的确认式批准/拒绝和 durable 事件时间线；事件对话禁止提交任意消息，拒绝后的 cancelled 状态会结束前后端 SSE。跨 org、缺权限、分页顺序与 token 重放通过真实 SQLite HTTP 测试；Plugin handler 重复 5 次/race 3 次/vet，AI HTTP/SQLite 重复 5 次/race 3 次，前端 typecheck、build 和 13 个测试文件/46 个用例通过。|
|G8：E2E 与收口|未开始|—|

## 当前边界

G0–G7 的合同、业务服务、隔离拓扑、资产能力、Incident/Approval/Execution 工作流和 Grafana 交互均已完成。下一步只进行跨容器真实告警、批准、CAS 执行、三重恢复验证、resolved 生命周期及三层审计 E2E，并在通过后收口 G8 文档证据。
