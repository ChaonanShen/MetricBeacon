...
|G4：assistant-mcp 资产与能力|进行中|基础拓扑已把 assistant-mcp 接入独立 order-ops 网络；Knowledge、Skill、Playbook 和精确 Alert Mapping 首版资产及严格 JSON Schema 已落地。合同门禁同时验证正向资产，以及健康值伪造、Fault/写能力暴露、自由执行步骤和缺少必需标签等反例。业务 Adapter、摘要固定和 MCP 注册待实现。|
...
G3 已完成。G4 已固定首批 Incident Tool 合同和版本化运行资产；下一步实现带摘要固定和精确匹配的文件资产 Adapter，以及 order-demo Mock/HTTP 只读 Port，并只在 incident profile 中注册这些能力。
