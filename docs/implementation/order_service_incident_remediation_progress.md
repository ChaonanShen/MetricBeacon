...
|G5：AI Core 只读 Incident|进行中|领域层已加入显式 private/org_incident Session、trigger Message、metric_analysis/incident_remediation Task discriminator、相互隔离的状态机、固定资产 IncidentPlan、结构化 Diagnosis 与严格 `0 -> 2` Intent；slow/dependency/healthy/证据不足只能 no-action，指标 Task 不能进入审批态。SQLite、告警入口与只读工作流仍在后续小步。|
|G6：审批、执行、验证与审计|未开始|—|
