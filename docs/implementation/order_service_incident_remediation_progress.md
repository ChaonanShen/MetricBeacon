...
|G6：审批、执行、验证与审计|进行中|Incident MCP 合同、共享 ApprovalEvidence、assistant-mcp 隔离的诊断/修复 Port、独立凭证、显式 remediation profile、两层执行 audit、固定 metrics view 和业务 probe已就绪。AI Core 已新增闭集处置领域及 SQLite `0008`，并增加只读 prepare Toolset：只向 `playbook.resume_run` 发送 opaque checkpoint 与已持久化 Diagnosis，只接受 needs-approval 的版本/epoch/摘要绑定 IntentDraft 或无 Intent 的 completed；参数中不存在 ApprovalEvidence、operation 或写动作。Adapter 测试重复 20 次、race 3 次及 vet 通过。下一步在工作流事务中冻结 Intent 并创建 Approval；执行/验证仍待完成。|
