...
|G6：审批、执行、验证与审计|进行中|Incident MCP 合同、共享 ApprovalEvidence、assistant-mcp 修复边界与验证工具、AI Core 闭集领域及 SQLite `0008` 已就绪。只读 prepare Toolset 只发送 opaque checkpoint 与已持久化 Diagnosis；工作流复验 capability/service/epoch/version/`0 -> 2`/60 秒观测/Playbook digest 后生成 canonical Intent digest，并在同一事务冻结 Intent、创建十分钟 pending Approval、推进 Task/checkpoint、追加 `intent.prepared`/`approval.requested`/status 事件。no-action 原子完成且无 Intent/Approval/Execution；无效 draft 失败且零 Approval；已落盘 Diagnosis 重启后直接继续 prepare、不重复四次诊断。工作流/Incident 重复 10 次、race 5 次、AI Core 全测试重复 3 次及 vet 通过。Approval 决策 API 和执行/验证仍待完成。|
