...
|G7：Plugin 与 Workbench|进行中|Plugin Backend 已增加组织 Incident list、Approval GET/POST 受控代理；可信 Grafana identity 为所有角色增加 `incidents:read`，仅真实 Admin 增加 `incidents:approve`，并在代理边界提前拒绝 Viewer 写请求。tenant/org/role/permission 伪造头均被覆盖，page token 与 Idempotency-Key 原样受控转发。AI Core 新增 `tenant + org + org_incident Session` 联表的 recent-activity Task 游标，token 绑定 tenant/org；相同可见性覆盖 Incident Session/Message/Task/Event，而 private 仍严格 owner-only。跨 org、缺权限、分页顺序与 token 重放通过真实 SQLite HTTP 测试；Plugin handler 重复 5 次/race 3 次/vet，AI HTTP/SQLite 重复 5 次/race 3 次通过。Workbench 尚待完成。|
|G8：E2E 与收口|未开始|—|
