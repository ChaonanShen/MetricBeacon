...
|G6：审批、执行、验证与审计|进行中|Incident MCP 合同已增加唯一 typed `0 -> 2` 写工具、固定 30 秒 Prometheus 恢复视图和固定业务 probe；Go/TypeScript 生成物可复现。新增共享 ApprovalEvidence codec，以 60 秒上限、HMAC 和精确 scope 防重放与串用。assistant-mcp 的诊断/修复 Port、独立凭证、显式 remediation profile、写前/写后 append-only audit、固定 metrics view 和业务 probe 已接入并通过重复、race、vet、Compose、Schema、生成和冷构建门禁。AI Core 已新增闭集处置领域：唯一 Intent 形状、十分钟 Approval 单次决定/到期、Execution started/unknown/reconcile/terminal 回执规则和固定 Audit action/outcome；领域全测试重复 20 次、race 3 次及 vet 通过。SQLite 持久化、Approval 应用服务和执行工作流仍待完成。|
