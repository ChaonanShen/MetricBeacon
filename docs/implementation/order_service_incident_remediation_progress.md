...
|G5：AI Core 只读 Incident|进行中|领域层已加入显式 private/org_incident Session、trigger Message、metric_analysis/incident_remediation Task discriminator、相互隔离的状态机、固定资产 IncidentPlan、结构化 Diagnosis 与严格 `0 -> 2` Intent；SQLite `0007` 已无伪 QueryPlan 地重建约束表，活跃 fingerprint、opaque checkpoint、V1/V6 升级和 FK 均有测试。AlertEvent 只保存有界摘要，原始 webhook/values/URL 不落库。新增 AI Core Incident Toolset Port/Adapter：resolve/start 只调用固定 Playbook 工具；诊断恰好调用 queue/worker/policy/recent 四个只读工具，输出摘要删除订单 ID，并确定性区分 stopped/slow/dependency/healthy/证据不足；只有 stopped 产生 restore 候选，工具名/身份透传/畸形结果和无 Fault/write/operation 调用均已测试。HMAC 入口与持久化工作流仍在后续小步。|
|G6：审批、执行、验证与审计|未开始|—|
